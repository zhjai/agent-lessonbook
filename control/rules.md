...
3. `state/` (`run_state`, `correction_journal`, `candidate_lessons`) is **working memory, not a source of truth**. It records what the agent believes, not what the system has verified.
...
6. **Lesson promotion cannot weaken enforcement ("no lesson laundering").** A lesson learned in a low-stakes run must never silently relax a stable rule or an external enforcement source (e.g. a separate completion gate's protected manifest). Such sources are human/CI-maintained and are NOT reachable by lesson promotion. Promoting a lesson that touches enforcement requires explicit human review at the enforcement tier, not the lesson tier.
